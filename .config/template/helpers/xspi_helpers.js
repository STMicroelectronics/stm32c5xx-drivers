/**
 * @file Helpers functions used for XSPI SW component
 * @license
  * @attention
  *
...
/**
  * Retrieve all the interruptions set by XSPI but not generated
  * @param {object} nvic_api Getter on NVIC api
  * @param {object} exti_api Getter on EXTI api (not used)
  * @param {object} resource Current resource
  * @param {object} config current configuration of the XSPI
  * @returns {object}
  */
function helper_xspi_get_irq_handler(nvic_api, exti_api, resource, config) {
  let result = [];
  try {
    console.info(`helper_xspi_get_irq_handler: resource=${resource}, config=${JSON.stringify(config)}`
    );
    ...
    console.error(`helper_xspi_get_irq_handler: ${e}`);
  }
  return result;
}
